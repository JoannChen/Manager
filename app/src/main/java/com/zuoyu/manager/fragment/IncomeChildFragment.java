...
import com.zuoyu.manager.adapter.IncomeAdapter;
 ...
 * </pre>
 */
public class IncomeChildFragment extends Fragment {
     ...
     * 【解析】近一周车场收费统计
     */
    private void parseAnalysisCharge() {
        ...
        ((BaseActivity)getActivity()).httpUtil.post(params, UrlManage.FC_ANALYSIS_CHARGE_URL, new HttpResult<IncomeEntity>() {
            @Override
            ...
            @Override
            public void onSuccess(IncomeEntity result) {

                ((IncomeFragment)getParentFragment()).setIncomeEntity(result);

                IncomeAdapter dap = new IncomeAdapter(result.getData().getChargeList(),getContext());
